...
function normalizedForm(form: NewSolutionInput): NewSolutionInput | null {
  ...
  return {
    ...form,
    performance: executionTime && memoryUsage ? { executionTime, memoryUsage } : undefined,
  };
}
...
export function Popup({
  ...
  function updatePerformance(key: "executionTime" | "memoryUsage", value: string) {
    setForm((current) => ({ ...current, performance: { executionTime: current.performance?.executionTime ?? "", memoryUsage: current.performance?.memoryUsage ?? "", [key]: value } }));
  }
  ...
  function beginDetectedCreate(problem: DetectedProblemInfo) {
    ...
    setSelectedRecord(null); setImportedFrom("SWEA 현재 페이지"); setError(""); setFeedback(""); setMode("create");
  }
  function beginEditorCreate(editor: SweaEditorInfo) {
    ...
    setSelectedRecord(null); setImportedFrom("SWEA 현재 풀이"); setError(""); setFeedback(""); setMode("create");
  }
  ...
  function beginEdit() {
    ...
    setImportedFrom(null); setError(""); setFeedback(""); setMode("edit");
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!normalized) { setError("실행시간과 메모리는 둘 다 입력하거나 둘 다 비워주세요."); return; }
    setSaving(true); setError(""); setFeedback("");
    try {
      if (mode === "edit" && selectedRecord) { const updated = await repository.update(selectedRecord.id, normalized); setSelectedRecord(updated); setRecords(await repository.list()); setMode("detail"); }
      else { await repository.create(normalized); setRecords(await repository.list()); setForm(EMPTY_FORM); setImportedFrom(null); setMode("list"); }
  ...
  async function copyCode() {
    if (!selectedRecord) return;
    setError(""); setFeedback("");
    try { await copyText(selectedRecord.code); setFeedback("코드가 복사되었습니다"); }
