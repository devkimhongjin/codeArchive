...
export function Popup({ repository = indexedDbSolutionRepository, requestPageContext, copyText = (text) => navigator.clipboard.writeText(text), openArchive = () => window.open(new URL("archive.html", window.location.href).href, "_blank"), confirmDelete = (message) => window.confirm(message) }: PopupProps) {
