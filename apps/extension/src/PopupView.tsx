...
import { PopupAuthPanel } from "./PopupAuthPanel";
...
import {
  automationGuidance,
  requestPopupAutomationState,
  setPopupAutomation,
  unavailableAutomationState,
...
function AutomationControls({
  ...
  return <section className="automation-controls" aria-labelledby="automation-controls-title">
    ...
    <fieldset disabled={disabled}>
      <legend className="sr-only">자동화 제어</legend>
...
export function Popup({
  ...
  openDashboard = () => window.open(CODEARCHIVE_DASHBOARD_ORIGIN, "_blank", "noopener,noreferrer"),
  openArchive = () => window.open(new URL("archive.html", window.location.href).href, "_blank", "noopener,noreferrer"),
  ...
  return <main className="popup" aria-labelledby="popup-title">
    ...
    {mode === "list" && <section className="dashboard-entry" aria-labelledby="dashboard-entry-title">
      <h2 id="dashboard-entry-title">웹 Dashboard</h2>
      <p>로그인과 서버 풀이 관리는 Dashboard에서 진행해요.</p>
