...
import { fetchSweaPerformance, parseSweaPerformance } from "./sweaPerformance";
...
function row({ user = nickname, time = "09:00", memory = "12,345 kb", execution = "123 ms", length = "3" } = {}) {
...
describe("SWEA performance", () => {
  ...
  it("uses UTF-8 bytes and rejects stale, ambiguous, malformed, and wrong-user rows", () => {
    expect(parseSweaPerformance(resultDocument(row({ length: "2" })), nickname, code, observedAt)).toBeNull();
