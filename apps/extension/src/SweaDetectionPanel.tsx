...
function DetectionContent({ result, onProblemPrefill, onEditorPrefill }: {
  ...
  if (result.status === "connected_page") {
    ...
    return (
      <div className="detection-result">
        ...
        {editor.status === "detected" && (
          <button className="secondary-button" type="button" onClick={() => onEditorPrefill(editor.editor)}>
            등록 폼에 채우기
          </button>
        )}
      </div>
...
export function SweaDetectionPanel({ requestContext = requestCurrentPageContext, onProblemPrefill, onEditorPrefill }: SweaDetectionPanelProps) {
  ...
  return (
    <section className="detection-card" aria-label="현재 페이지 감지">
      ...
      {state.status === "connected" && <DetectionContent result={state.result} onProblemPrefill={onProblemPrefill} onEditorPrefill={onEditorPrefill} />}
    </section>
  );
}
