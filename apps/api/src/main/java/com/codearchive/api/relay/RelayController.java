...
import com.codearchive.api.auth.*; import com.codearchive.api.common.ApiError; import com.codearchive.api.solution.*; import com.codearchive.api.automation.GithubAutomationService; import com.codearchive.api.settings.*;
...
@RestController @RequestMapping("/api/relay") public class RelayController {
 ...
 @PostMapping("/grants") @Transactional public ResponseEntity<?> issue(Authentication auth,@RequestBody GrantRequest request){
   AppUser user=GithubAuthentication.identity(auth).flatMap(i->users.findByGithubId(i.githubId())).orElse(null); if(user==null)return ResponseEntity.status(401).body(new ApiError("Authentication is required"));
 ...
 @DeleteMapping("/grants/{deviceId}") @Transactional public ResponseEntity<?> revoke(Authentication auth,@PathVariable String deviceId){
   AppUser user=GithubAuthentication.identity(auth).flatMap(i->users.findByGithubId(i.githubId())).orElse(null); if(user==null)return ResponseEntity.status(401).body(new ApiError("Authentication is required"));
