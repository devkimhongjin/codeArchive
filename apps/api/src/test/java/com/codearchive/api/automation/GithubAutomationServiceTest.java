...
class GithubAutomationServiceTest {
  ...
  @Test void targetChangeRefreshesBoundaryButThemeOnlyChangeDoesNot() throws Exception {
    ...
    setting.apply(new SettingsRequest(1,"n","n",false,false,"{number}","{number}","one-light","dracula",true,true,1L,"owner","repo","main",null));
    assertThat(setting.getAutomationEnabledAt()).isEqualTo(before);
    setting.apply(new SettingsRequest(1,"n","n",false,false,"{number}","new/{number}","one-light","dracula",true,true,1L,"owner","repo","main",null));
