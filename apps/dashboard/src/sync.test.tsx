...
it('persists an offline inline code-view theme and seeds the next local archive render', async () => {
  ...
  fireEvent.change(screen.getByLabelText('코드 보기 테마'), { target: { value: 'vitesse-light' } })
  await waitFor(() => expect(JSON.parse(localStorage.getItem('codearchive-local-code-themes') ?? '{}').lightTheme).toBe('vitesse-light'))
  first.unmount()
  const second = render(<App />)
  await screen.findAllByText('테스트 풀이')
  expect((screen.getByLabelText('코드 보기 테마') as HTMLSelectElement).value).toBe('vitesse-light')
  fireEvent.change(screen.getByLabelText('코드 보기 테마'), { target: { value: 'dracula' } })
  await waitFor(() => expect(localStorage.getItem('codearchive-code-theme-mode')).toBe('dark'))
