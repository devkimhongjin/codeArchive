...
function GitHubUploadBody({ solution, client, syncEligible, onSessionExpired }: { solution: DashboardSolution | null; client: GitHubClient; syncEligible: boolean; onSessionExpired: () => void }) {
  ...
  return <>
    ...
    <section className="github-manual" aria-label="한 번 커밋">
      ...
      {receipt && <div className="github-result" role="status"><strong>{receipt.status === "SUCCEEDED" ? "커밋 완료" : receipt.status === "UNKNOWN" ? "전송 결과 확인 필요 · 재전송하지 마세요" : receipt.status === "REJECTED" ? "전송 전 중단됨" : "요청 상태 확인 필요"}</strong>{receipt.commitUrl && <a href={receipt.commitUrl} target="_blank" rel="noopener noreferrer">GitHub 커밋 보기</a>}<button disabled={busy} onClick={() => void perform(s => client.result(receipt.intentId, s), setReceipt)}>전송 결과 확인</button></div>}
    </section>
