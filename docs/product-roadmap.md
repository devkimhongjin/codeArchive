...
사용자 추가 결정에 따라 **각 문제의 ‘다른 풀이 보기’는 같은 플랫폼·문제의 본인 ACCEPTED 풀이를 PUBLIC으로 공개한 로그인 사용자에게만 허용**한다. 여기서 공개는 같은 문제를 성공적으로 풀고 공개한 사람 사이의 상호 공유이며, 무인증 인터넷 전체 공개를 뜻하지 않는다. 목록·상세·공유 링크·개수·댓글·좋아요 API 모두 이 자격을 검사한다. 비자격 사용자는 개수/미리보기도 조회할 수 없고, 마지막 자격 풀이를 비공개/삭제하면 접근과 캐시를 즉시 해제한다. 다른 플랫폼의 같은 번호는 별개 문제다. 수동 입력/편집만으로 성공 자격을 얻지 않도록 ACCEPTED provenance 계약을 먼저 고정한다.
...
인계 요청: “#143 PR의 exact head, CI 및 자체검토 결과를 확인하고 #37/#84의 사용자 확인과 남은 E2E 증거를 구분하세요. 완료 조건이 충족되면 #86 정리를 분리하고, 신규 기능은 사용자 결정에 따라 #45 → #141 → #142 순서로 진행하세요. 다음 기능 설계는 공개/비공개 전환·공유 링크와 같은 문제의 ACCEPTED 풀이 공개자에게만 허용하는 ‘다른 풀이 보기’부터 시작하세요. 병합과 배포는 별도 승인입니다.”
